 ...
 * the "degrades gracefully when unconfigured" path, and the found/not_found/error
 * paths against a mocked fetch — so the suite doesn't depend on a real network call
 * or a real GUID to run. Every configured-vs-unconfigured scenario explicitly forces
 * env.abrAbnLookup.guid via jest.doMock rather than relying on the ambient .env's
 * actual state, since a developer's own machine may legitimately have a real GUID
 * set for real dev use (env.ts reads process.env once at import time, so a test-only
 * process.env mutation wouldn't reach an already-resolved env.guid anyway).
 */
import { isValidAbnChecksum } from '../../src/modules/businesses/providers/abn-lookup.provider';

async function lookupWithGuid(rawAbn: string, guid: string) {
...
describe('lookupAbn', () => {
  ...
  it('short-circuits on an invalid checksum without making a network call', async () => {
    ...
    const outcome = await lookupWithGuid('12345678901', '');
    expect(outcome).toEqual({ status: 'invalid_abn' });
    expect(fetchSpy).not.toHaveBeenCalled();
  });

  it('returns not_configured when no GUID is configured', async () => {
  ...
  describe('with a GUID configured', () => {
    it('parses a found response', async () => {
      ...
      const outcome = await lookupWithGuid('51824753556', 'test-guid');
